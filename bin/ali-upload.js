...
Main.startUpload = function(oss) {
	haxe_Log.trace("检索目录...",{ fileName : "src/Main.hx", lineNumber : 49, className : "Main", methodName : "startUpload"});
	Main.readDir(Main.targetDir);
	haxe_Log.trace("总文件数量：" + Main.allfile.length + "个，开始上传...",{ fileName : "src/Main.hx", lineNumber : 51, className : "Main", methodName : "startUpload"});
	var tcount = 30;
	var len = Main.allfile.length / tcount | 0;
	if(len == 0) {
		len = 1;
	}
	haxe_Log.trace(tcount + "线程上传：" + len,{ fileName : "src/Main.hx", lineNumber : 56, className : "Main", methodName : "startUpload"});
	var _g = 0;
	var _g1 = tcount;
	while(_g < _g1) {
		var i = _g++;
...
Main.onSuccess = function() {
	haxe_Log.trace("上传完成，远程地址：https://static.kdyx.cn/" + Main.aliyunTargetDir,{ fileName : "src/Main.hx", lineNumber : 70, className : "Main", methodName : "onSuccess"});
	var date = DateTools.format(new Date(new Date().getTime() - Main.startTime),"%M:%S");
	haxe_Log.trace("总耗时：" + date + "秒",{ fileName : "src/Main.hx", lineNumber : 72, className : "Main", methodName : "onSuccess"});
};
Main.upload = async function(oss,start,end) {
	...
	while(_g < _g1) {
		...
		try {
			...
			Main.allUploadCount++;
			haxe_Log.trace("上传进度[" + Main.allUploadCount + "/" + Main.allfile.length + "]" + bf + "% :" + file,{ fileName : "src/Main.hx", lineNumber : 91, className : "Main", methodName : "upload"});
			if(Main.allUploadCount == Main.allfile.length) {
				Main.onSuccess();
			}
		} catch( _g2 ) {
			var e = haxe_Exception.caught(_g2);
			haxe_Log.trace("上传文件失败：" + file,{ fileName : "src/Main.hx", lineNumber : 96, className : "Main", methodName : "upload", customParams : ["原因：" + Std.string(e)]});
		}
	}
};
